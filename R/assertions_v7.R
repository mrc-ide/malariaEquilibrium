...
#' @noRd
assert_vector_numeric <- function(x, name = paste(deparse(substitute(x)), collapse = "")) {
  assert_numeric(x, name = name)
...
#' @noRd
assert_vector_pos <- function(x, name = paste(deparse(substitute(x)), collapse = "")) {
  assert_pos(x, name = name)
  assert_vector(x, name = name)
  return(TRUE)
}
